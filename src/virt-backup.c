...
/* Function to get physical memory. Required for LZMA decompression routine. */
...
/* Simple check whether flag is being set or not */
int getFlagSet(unsigned long flags, int flag) {
	return (flags & flag) ? 1 : 0;
}

/* Function to get maximum temperature which is saved in the latter bits of the flag value */
...
/* Process the sensor action once the set temperature is being met */
void doSensorAction(char *sensor_action, int val, int elapsed) {
	...
	if (strstr(tmp, ":") != NULL) {
		...
		if (flags & FLAG_DEBUG)
			DPRINTF("doSensorsAction: action = %s, params = %s\n", action, params);
...
/* Function to set the maximum temperature read from the string format */
unsigned int setMaximumTemperature(char *str) {
	...
	return ((val > 0) && (errno == 0)) ?
		FLAG_SENSOR_HAVEMAX | ( val << MAX_TEMP_SHIFT ) : 0;
}

/* Gets information whether connection is local or not */
...
/* Function to get the selinux policy for file */
...
/* Function to set the selinux policy for file */
...
/* Function to get the file ownership or question mark (?) if information cannot be found */
char *getOwnership(char *fileName)
{
	...
	value = (char *)malloc( (strlen(buf) + 3) * sizeof(char));
	memset(value, 0, (strlen(buf) + 3));
	strncpy(value, buf, strlen(buf));
	return value;
}

/* Set the ownership for file by user:group format value */
...
/* Get the time value and save the result into buf */
...
/* Gets the file size for the file defined by fileName */
unsigned long long getFileSize(char *fileName)
{
	...
	if (fd < 0) {
		DPRINTF("%s: Error when opening %s for reading (%d)\n", __FUNCTION__, fileName, errno);
		return 0;
	}
...
{
	DPRINTF("%s: Entering function for path %s...\n", __FUNCTION__, path);
...
struct tFiles *getRestoreXml(char *xmlFile, int *numFiles)
{
	xmlDocPtr doc;
	xmlNodePtr cur, cur2;
	int num = -1;
	...
	files = malloc ( sizeof(struct tFiles) );
		if (!files)
			return NULL;
	...
	while (cur != NULL) {
			if ((!xmlStrcmp(cur->name, (const xmlChar *)"file"))) {
				...
				num++;
				files = realloc( files, (num+1) * sizeof(tFiles) );
			}
...
out_phys:
	...
	if (getuid() == 0) {
		...
		for (i = startIdx; i < numFiles; i++) {
			...
			if (mSize <= 0) {
				fprintf(stderr, "Error: Block device %s does not exist or is not accessible!\n", file);
				continue;
			}
	...
	for (i = 0; i < numFiles; i++)
	{
		...
		snprintf(tmpFile, 1024, "%s/%s", savePath, fileName);
		 ...
		 if (!compress) {
			...
			DPRINTF("Error: Unsupported compression option (possible bug)\n");
			return -ENOTSUP;
	        }
...
int restoreGuests(virConnectPtr cp, char *pathFrom, char *path, struct tFiles *files, int num)
{
	int i, from, to;
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(action, "restore") == 0) {
        ...
        struct tFiles *files = getRestoreXml(xml, &num);
